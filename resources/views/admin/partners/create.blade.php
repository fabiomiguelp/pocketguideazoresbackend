...
@section('title', 'Create Partner | MyTripTaylor Admin')
@section('title-sub', 'Partners')
@section('pagetitle', 'Create Partner')
                ...
                <form action="{{ route('admin.partners.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('trip_category_id') is-invalid @enderror" id="trip_category_id" name="trip_category_id" required>
                            ...
                            <option value="{{ $cat->id }}" {{ old('trip_category_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('budget_level_id') is-invalid @enderror" id="budget_level_id" name="budget_level_id" required>
                            ...
                            <option value="{{ $bl->id }}" {{ old('budget_level_id') == $bl->id ? 'selected' : '' }}>{{ $bl->name }} ({{ $bl->min_budget }}{{ $bl->max_budget ? '-'.$bl->max_budget : '+' }}€)</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4">
