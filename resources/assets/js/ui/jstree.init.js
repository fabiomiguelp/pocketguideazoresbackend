    ...
    const createNode = (item) => {
        ...
        if (item.children && item.children.length > 0) {
            ...
            a.addEventListener("click", (event) => {
                ...
                event.target.closest("li").querySelectorAll('.selected').forEach(el => el.classList.remove('selected'));

                // Remove active class from all links
                event.target.closest("li").querySelectorAll('.active').forEach(el => el.classList.remove('active'));
